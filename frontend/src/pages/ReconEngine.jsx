...
export default function ReconEngine() {
  ...
  const [semgrep, setSemgrep] = useState([]);
  ...
  const topTarget = targets.length > 0 ? targets[0] : null;
        ...
        <div className="card accent-left-red recon__hero">
          {topTarget ? (
            ...
            <div className="p-6 text-center text-muted">
              <p className="text-sm font-semibold mb-2">No Recon Targets Identified Yet</p>
              <p className="text-xs text-muted">Run a scan on a target repository to populate AST function call trees & Semgrep static findings.</p>
            </div>
          )}
        </div>
      </motion.div>
