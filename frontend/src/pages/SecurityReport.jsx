...
export default function SecurityReport() {
  ...
  const vul = report?.vulnerability || null;
        ...
        <>
          <motion.div className="card report__meta-strip section-gap--sm" {...anim(0.06)}>
            <div className="report__meta-item">
              ...
              <span className="report__meta-val mono font-semibold">{report.id || 'N/A'}</span>
            </div>
            <div className="report__meta-item">
              <span className="report__meta-key">Target Binary</span>
              <span className="report__meta-val font-semibold">{scan?.target_name || report.target || 'N/A'}</span>
            </div>
            <div className="report__meta-item">
              <span className="report__meta-key">Vulnerability</span>
              <div className="flex items-center gap-2">
                <span className="badge badge--critical">{vul?.cvssScore || 0} CRITICAL</span>
                <span className="mono text-xs" style={{ color: 'var(--t2)' }}>{vul?.cweId || 'CWE'}</span>
              </div>
            </div>
            <div className="report__meta-item">
              <span className="report__meta-key">Recommendation</span>
              <span className={`badge ${report.recommendation === 'SAFE' ? 'badge--green' : report.recommendation === 'REVIEW' ? 'badge--amber' : 'badge--danger'} mono font-bold`}>
                {report.recommendation || 'PENDING'}
              </span>
            </div>
            <div className="report__meta-item">
              <span className="report__meta-key">AI Confidence</span>
              <span className="report__meta-val mono text-amber font-bold" style={{ fontSize: 16 }}>{report.confidence || 0}%</span>
            </div>
          </motion.div>
          ...
          <motion.div className="card accent-left-amber section-gap--sm" {...anim(0.08)}>
            ...
            <p className="text-sm text-secondary" style={{ lineHeight: 1.6 }}>
              {report.rootCause || 'No root cause analysis recorded yet.'}
            </p>
          </motion.div>
        </>
      ) : (
        <div className="card p-6 text-center text-muted section-gap--sm">
          <p className="text-sm font-semibold mb-1">No Active Security Report Generated</p>
          <p className="text-xs">Security reports and human safety gate controls are generated upon completing a full scan pipeline.</p>
        </div>
      )}
